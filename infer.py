...
def get_info_single_page(car_url):
    ...
    result = requests.get(f"{car_url}", timeout = 30)
...
file_bonbanh = "id_files.txt"

# Mở tệp tin và đọc từng dòng
def process_line(line):
    link = line.strip().replace("\n", "")
    id = link.split('-')[-1]
    if id not in completed_ids or id not in failed_id:
        try:
            post_dict = get_info_single_page(link)
            if post_dict != None:
                idx = link.split("-")[-1]
                export_json(post_dict, json_path=f"json_manh/{idx}.json")
                print(f'Crawl {link} successfully!')
            else:
                print(f'{link} - hết hạn rồi')
                with open('time_out_files.txt', "a+") as file:
                    file.write(link + "\n")
...
with open('time_out_files.txt', 'r') as file:
...
for filename in os.listdir('json_manh'):
    if filename.endswith(".json"):
        idx = filename.split('.')[0]
        completed_ids.append(idx)

# Mở tệp tin và đọc từng dòng
with open(file_bonbanh, 'r') as file:
    lines = file.readlines()

# Sử dụng ThreadPoolExecutor với 5 thread
